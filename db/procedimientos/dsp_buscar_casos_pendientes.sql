...
CREATE PROCEDURE `dsp_buscar_casos_pendientes`(pCadena varchar(50), pIdEstudio int, pDocumento varchar(100), pTelefono varchar(100), pOffset int, pLimit int, pVisitado char(1),
                                                ...
                                                pOrigenes json, pFecha varchar(45), pFechaDesde varchar(45), pFechaHasta varchar(45))
BEGIN
	/*
    Permite buscar tipos de caso filtrándolos por una cadena de búsqueda. Ordena por TipoCaso.
    */
    DECLARE pCant int;
    DECLARE pCantEstados, pCantOrigenes, pCantFechasAlta, pCantFechasVisitado json;
    ...
    SET pCantEstados =  (SELECT 	JSON_OBJECTAGG(EstadoCasoPendiente, Cant)
                        FROM 	(SELECT ecp.EstadoCasoPendiente, COUNT(cp.IdEstadoCasoPendiente) Cant
                                ...
                                WHERE(
                                    ...
                                    (JSON_CONTAINS(pOrigenes, CONCAT('"', o.Origen, '"')) = 1 OR JSON_EXTRACT(pOrigenes, '$[0]') IS NULL) AND
                                    (
                                        (
                                            (pFecha IS NULL OR pFecha = '') AND (
                                                JSON_CONTAINS(pFechasAlta, CONCAT('"', DATE(cp.FechaAlta), '"')) = 1 OR
                                                JSON_EXTRACT(pFechasAlta, '$[0]') IS NULL
                                            )
                                        ) OR
                                        pFecha = 'RANGO' AND DATE(cp.FechaAlta) BETWEEN DATE(pFechaDesde) AND DATE(pFechaHasta)
                                    ) AND
    ...
    SET pCantOrigenes =  (SELECT 	JSON_OBJECTAGG(Origen, Cant)
                        FROM 	(SELECT o.Origen, COUNT(o.IdOrigen) Cant
                                FROM CasosPendientes cp
                                LEFT JOIN EstadosCasoPendiente ecp USING(IdEstadoCasoPendiente)
                                LEFT JOIN   Origenes o USING(IdOrigen)
                                WHERE(
    ...
    SELECT DISTINCT	cp.*, c.Caratula, ct.IdChat, ecp.EstadoCasoPendiente, o.Origen, pCant Cant, (SELECT		JSON_OBJECT(
                ...
                pCantFechasAlta CantFechasAlta, pCantFechasVisitado CantFechasVisitado, pCantEstados CantEstados, pCantOrigenes CantOrigenes, cp.IdUsuarioVisita, CONCAT(u.Apellidos, ', ', u.Nombres) UsuarioVisita
    FROM		CasosPendientes cp
    LEFT JOIN   EstadosCasoPendiente ecp USING(IdEstadoCasoPendiente)
    LEFT JOIN   Origenes o USING(IdOrigen)
