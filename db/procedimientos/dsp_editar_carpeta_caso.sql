DROP PROCEDURE IF EXISTS `dsp_editar_carpeta_caso`;
DELIMITER $$
CREATE PROCEDURE `dsp_editar_carpeta_caso`(pJWT varchar(500), pIdCaso bigint, pIdCarpetaCaso int, pNombre varchar(100))
PROC: BEGIN
	/*
    ...
    */
    ...
    IF pNombre IS NULL OR TRIM(pNombre) = '' THEN
		SELECT 'Debe indicar un nombre para la carpeta.' Mensaje;
        LEAVE PROC;
	END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM CarpetasCaso WHERE IdCarpetaCaso = pIdCarpetaCaso) THEN
        ...
        LEAVE PROC;
	END IF;
    IF EXISTS (SELECT 1 FROM CarpetasCaso WHERE Nombre = pNombre AND IdCarpetaCaso != pIdCarpetaCaso AND IdCaso = pIdCaso) THEN
    ...
    START TRANSACTION;
        UPDATE  CarpetasCaso
        SET     Nombre = pNombre
        WHERE   IdCarpetaCaso = pIdCarpetaCaso;
