DROP PROCEDURE IF EXISTS `dsp_activar_usuario_estudio`;
DELIMITER $$
CREATE PROCEDURE `dsp_activar_usuario_estudio`(pJWT varchar(500), pIdEstudio int, pIdUsuario int,
			pIP varchar(40), pUserAgent varchar(255), pApp varchar(50))
PROC: BEGIN
	/*
    Permite cambiar el estado de un abogado de un estudio a Activo. 
    ...
    */
    ...
    IF NOT EXISTS (SELECT IdUsuario FROM Usuarios WHERE IdUsuario = pIdUsuario) THEN
        ...
        LEAVE PROC;
	END IF;
    IF EXISTS (SELECT IdUsuario FROM UsuariosEstudio WHERE IdUsuario = pIdUsuario AND IdEstudio != pIdEstudio AND Estado = 'A') THEN
		SELECT 'El usuario ya está activo en otro estudio.' Mensaje;
        LEAVE PROC;
	END IF;
    IF NOT EXISTS (SELECT IdUsuario FROM UsuariosEstudio WHERE IdUsuario = pIdUsuario AND IdEstudio = pIdEstudio) THEN
        ...
        LEAVE PROC;
	END IF;
    IF (SELECT Estado FROM UsuariosEstudio WHERE IdUsuario = pIdUsuario AND IdEstudio = pIdEstudio) = 'A' THEN
		SELECT 'OK' Mensaje;
        LEAVE PROC;
	END IF;
    START TRANSACTION;
		...
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'ACTIVAR', 'A', UsuariosEstudio.* 
        ...
        -- Después
		INSERT INTO aud_UsuariosEstudio
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'ACTIVAR', 'D', UsuariosEstudio.* 
        FROM UsuariosEstudio WHERE IdEstudio = pIdEstudio AND IdUsuario = pIdUsuario;
