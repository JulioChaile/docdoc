DROP PROCEDURE IF EXISTS `dsp_alta_carpeta_caso`;
DELIMITER $$
CREATE PROCEDURE `dsp_alta_carpeta_caso`(pJWT varchar(500), pIdCaso bigint, pNombre varchar(100))
PROC: BEGIN
	/*
    ...
    */
    ...
    IF NOT EXISTS (SELECT IdCaso FROM Casos WHERE IdCaso = pIdCaso) THEN
        ...
        LEAVE PROC;
	END IF;
    IF EXISTS (SELECT 1 FROM CarpetasCaso WHERE Nombre = pNombre AND IdCaso = pIdCaso) THEN
		SELECT 'El nombre indicado ya se encuentra en uso.' Mensaje;
        LEAVE PROC;
	END IF;
    START TRANSACTION;
        INSERT INTO CarpetasCaso VALUES(0, pIdCaso, pNombre);
