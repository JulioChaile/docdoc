DROP PROCEDURE IF EXISTS `dsp_listar_permisos_rol`;
DELIMITER $$
CREATE PROCEDURE `dsp_listar_permisos_rol`(pIdRol int)
BEGIN
	/*
    ...
    */
    ...
    CREATE TEMPORARY TABLE Arbol ENGINE = MEMORY
		...
		FROM	Permisos p LEFT JOIN (select * from PermisosRol where IdRol = pIdRol) pr ON p.IdPermiso = pr.IdPermiso
		WHERE	p.Estado='A';
    -- Nivel 1
    SET pNivel = 1;
