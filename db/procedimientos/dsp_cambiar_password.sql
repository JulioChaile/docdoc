DROP PROCEDURE IF EXISTS `dsp_cambiar_password`;
DELIMITER $$
CREATE PROCEDURE `dsp_cambiar_password`(pModo char(1), pToken varchar(500),
    ...
    */
    ...
    START TRANSACTION;
        ...
        IF pModo = 'R' THEN
			...
			SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'REHASH', 'A', Usuarios.* 
            ...
            -- Auditoría
			INSERT INTO aud_Usuarios
			SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'REHASH', 'D', Usuarios.* 
			FROM Usuarios WHERE IdUsuario = pIdUsuario;
        ...
        IF pModo IN ('U','C') THEN
            ...
            -- Auditoría
			INSERT INTO aud_Usuarios
			SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'CAMBIAR#PASS', 'A', Usuarios.* 
			FROM Usuarios WHERE IdUsuario = pIdUsuario;
            ...
            -- Auditoría
			INSERT INTO aud_Usuarios
			SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'CAMBIAR#PASS', 'D', Usuarios.* 
			FROM Usuarios WHERE IdUsuario = pIdUsuario;
