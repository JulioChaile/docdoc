DROP PROCEDURE IF EXISTS `dsp_darbaja_caso`;
DELIMITER $$
CREATE PROCEDURE `dsp_darbaja_caso`(pJWT varchar(500), pIdCaso bigint,
			pIP varchar(40), pUserAgent varchar(255), pApp varchar(50))
PROC: BEGIN
	/*
    Permite cambiar el estado de un proyecto a Baja. Devuelve OK o el mensaje de error en Mensaje.
    */
    ...
    IF NOT EXISTS (SELECT IdCaso FROM Casos WHERE IdCaso = pIdCaso) THEN
        ...
        LEAVE PROC;
	END IF;
    IF EXISTS (SELECT IdCaso FROM Casos WHERE IdCaso = pIdCaso AND Estado = 'B') THEN
    ...
    START TRANSACTION;
		...
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'BAJA', 'A', Casos.* 
        ...
        -- Después
		INSERT INTO aud_Casos
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'BAJA', 'D', Casos.* 
        FROM Casos WHERE IdCaso = pIdCaso;
