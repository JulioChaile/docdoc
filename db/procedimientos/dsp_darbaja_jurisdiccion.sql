DROP PROCEDURE IF EXISTS `dsp_darbaja_jurisdiccion`;
DELIMITER $$
CREATE PROCEDURE `dsp_darbaja_jurisdiccion`(pJWT varchar(500), pIdJurisdiccion int, 
		pIP varchar(40), pUserAgent varchar(255), pApp varchar(50))
PROC: BEGIN
	/*
    Permite cambiar el Estado de una jurisdiccion a Baja. 
    ...
    */
    ...
    IF NOT EXISTS (SELECT IdJurisdiccion FROM Jurisdicciones WHERE IdJurisdiccion = pIdJurisdiccion) THEN
        ...
        LEAVE PROC;
	END IF;
    IF (SELECT Estado FROM Jurisdicciones WHERE IdJurisdiccion = pIdJurisdiccion) = 'B' THEN
		SELECT 'OK' Mensaje;
		LEAVE PROC;
	END IF;
    START TRANSACTION;
		...
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'DARBAJA', 'A', Jurisdicciones.* 
        ...
        INSERT INTO aud_Jurisdicciones
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'DARBAJA', 'D', Jurisdicciones.* 
        FROM Jurisdicciones WHERE IdJurisdiccion = pIdJurisdiccion;
