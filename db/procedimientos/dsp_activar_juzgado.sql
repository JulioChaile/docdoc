DROP PROCEDURE IF EXISTS `dsp_activar_juzgado`;
DELIMITER $$
CREATE PROCEDURE `dsp_activar_juzgado`(pJWT varchar(500), pIdJuzgado int,
		pIP varchar(40), pUserAgent varchar(255), pApp varchar(50))
PROC: BEGIN
	/*
    Permite cambiar el estado de un juzgado a Activo. 
    ...
    */
    ...
    IF NOT EXISTS (SELECT IdJuzgado FROM Juzgados WHERE IdJuzgado = pIdJuzgado) THEN
        ...
        LEAVE PROC;
	END IF;
    IF (SELECT Estado FROM Juzgados WHERE IdJuzgado = pIdJuzgado) = 'A' THEN
		SELECT 'OK' Mensaje;
		LEAVE PROC;
	END IF;
    START TRANSACTION;
		...
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'ACTIVAR', 'A', Juzgados.* 
        ...
        INSERT INTO aud_Juzgados
		SELECT 0, NOW(), pUsuario, pIP, pUserAgent, pApp, 'ACTIVAR', 'D', Juzgados.* 
        FROM Juzgados WHERE IdJuzgado = pIdJuzgado;
